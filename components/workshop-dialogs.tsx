...
import { CommerceSettings } from './template-store';
...
import { ArrowUpRight, Wallet, Check, Plus, X, Download, Code2, Copy, ShieldCheck, Clock3, KeyRound, Users, MessageSquare } from 'lucide-react';
...
export function WalletPanel({wallet,refresh,ar}:{wallet:any;refresh:()=>Promise<void>;ar:boolean}) {
 ...
 return <div className="wallet-panel" dir={ar?'rtl':'ltr'}>
  <div className="wallet-balance"><div><span className="eyebrow">{t('PAID & SUBSCRIPTION BALANCE','رصيد الشحن والاشتراك')}</span><h2 dir="ltr">{money(wallet.total||0)}</h2><span className="mode-tag">{wallet.mode==='test'?t('TEST BALANCE · NO CASH VALUE','رصيد تجريبي · دون قيمة مالية'):t('USD SERVICE BALANCE','رصيد خدمات بالدولار')}</span></div><Wallet size={42}/></div>
  ...
  <div className="template-welcome"><div><strong>{t('Welcome template credit','رصيد القوالب الترحيبي')}: <span dir="ltr">{money(wallet.promotion?.balance||0)}</span></strong><p>{t('Separate, non-cash credit for the ready-made templates under New project. It cannot fund image generation.','رصيد منفصل وغير نقدي للقوالب الجاهزة في «مشروع جديد». لا يُستخدم لتوليد الصور.')}</p></div>{wallet.promotion?.available&&<Button variant="outline" disabled={busy} onClick={()=>action(()=>api('/api/wallet/welcome',{method:'POST',body:{}}))}>{t('Claim $10 welcome credit','استلم 10 دولارات ترحيبية')}</Button>}</div>
  <div className="wallet-explainer"><article><h3>{t('Wallet = pay for use','المحفظة = دفع مقابل الاستخدام')}</h3><p>{t('Top up without a subscription. Each approved generation deducts its quoted price; manual edits do not consume balance.','اشحن دون اشتراك. كل عملية توليد توافق عليها تخصم السعر المعروض؛ التعديل اليدوي لا يستهلك الرصيد.')}</p></article><article><h3>{t('Subscription = balance + capabilities','الاشتراك = رصيد + مزايا')}</h3><p>{t('Three plans add monthly service balance, more storage and higher operation limits. Pro adds brand kits; Studio adds team roles and review.','ثلاث باقات تضيف رصيد خدمات شهريًا ومساحة أكبر وحدود عمليات أعلى. تضيف Pro هويات المشاريع، وتضيف Studio صلاحيات الفريق والمراجعة.')}</p></article></div><p className="wallet-help">{t('Your wallet pays for generation. Your subscription adds storage, reusable resources and team capabilities. Manual editing, saving and export have no action fee within storage and request limits.','المحفظة تسدد تكلفة التوليد. الاشتراك يضيف مساحة ومكتبة إنتاج وقدرات للفريق. التحرير اليدوي والحفظ والتصدير دون رسم للعملية وضمن حدود المساحة والطلبات.')}</p>
  <div className="usage-meter"><div className="row between"><strong>{t('Current workspace','المساحة الحالية')}: {ent.name}</strong><span>{(used/1000000000).toFixed(2)} / {ent.storageBytes/1000000000} GB</span></div><Progress className="account-meter" value={Math.min(100,used/ent.storageBytes*100)}/><p className="muted">{t('Changing plans keeps your existing projects and files. Files and retained project versions count toward storage. Archiving does not remove saved history.','تغيير الباقة يحتفظ بمشاريعك وملفاتك. تُحسب الملفات ونسخ المشروع المحفوظة ضمن المساحة. الأرشفة لا تحذف السجل.')}</p></div>
...
export function ExportPanel({ project, ar }: {
    project: Project;
    ar: boolean;
}) { const [format, setFormat] = useState('source'), [withAdmin,setWithAdmin] = useState(false), [sceneId, setSceneId] = useState(project.graph.entries[0] || project.graph.pieces.find(p => p.type === 'page')?.id || ''), [duration, setDuration] = useState(8), [busy, setBusy] = useState(false), [progress, setProgress] = useState(0), [abort, setAbort] = useState<AbortController | null>(null), t = (e: string, a: string) => ar ? a : e; const scenes = project.graph.pieces.filter(p => p.type === 'page');
 const [sourceFiles,setSourceFiles]=useState<{name:string;content:string}[]>([]),[sourcePath,setSourcePath]=useState('README.md');
 useEffect(()=>setSourceFiles([]),[withAdmin,sceneId,project.revision]);
 async function prepareSource(){const {portableSources}=await import('@/lib/source-assets');const assets=await assetBundle(project),files=portableFiles({format:'vorlda-project',version:1,name:project.name,graph:project.graph,assets,revision:project.revision,entry:sceneId,runtime:{admin:withAdmin}},portableSources);files.push({name:'manifest.json',content:await portableManifest(files)});return files;}
 async function inspectSource(){setBusy(true);try{setSourceFiles(await prepareSource());}catch(e){toast.error((e as Error).message)}finally{setBusy(false)}}
 const viewedFile=sourceFiles.find(f=>f.name===sourcePath);
 async function run() { setBusy(true); setProgress(0); const controller = new AbortController(); setAbort(controller); try {
    ...
    setBusy(false);
    setAbort(null);
} } return <div className="export-panel"><div className="notice">{t(`Exports use applied revision ${project.revision}. Draft changes must be reviewed and applied first.`, `يستخدم التصدير النسخة المعتمدة ${project.revision}. راجع تعديلات المسودة واعتمدها أولًا.`)}</div><Choice label={t('Export format','صيغة التصدير')} value={format} onChange={setFormat} options={[{value:'source',label:t('App source + forms server (.zip)','كود التطبيق وخادم النماذج (.zip)')},{ value: 'project', label: t('Project + original assets (.vorlda.json)', 'المشروع والملفات الأصلية (.vorlda.json)') }, { value: 'html', label: t('Application · standalone HTML', 'التطبيق · ملف HTML مستقل') }, { value: 'png', label: t('Composition · PNG image', 'التصميم · صورة PNG') }, { value: 'film', label: t('Film · video with audio', 'فيلم · فيديو مع الصوت') }]}/>{format !== 'project' && <Choice label={t('Scene or entry','المشهد أو نقطة البداية')} value={sceneId} onChange={setSceneId} options={scenes.map(p => ({ value: p.id, label: p.name }))}/>}<>{format === 'source'&&<Toggle label={t('Include a separate owner admin console','إضافة لوحة إدارة مستقلة لمالك التطبيق')} checked={withAdmin} onChange={setWithAdmin}/>}</><p className="muted">{format === 'source' ? t('Native VORLDA code, build scripts and a Node/SQLite forms backend. Optional owner administration requires first-use password replacement. Customer accounts, billing and AI are not included. External media may still need internet.','كود VORLDA الأصلي وملفات البناء وخادم Node/SQLite للنماذج. لوحة المالك اختيارية وتلزم تغيير كلمة المرور الأولية. حسابات العملاء والدفع والذكاء الاصطناعي غير مضمنة. قد تحتاج الوسائط الخارجية الإنترنت.') : format === 'html' ? t('Assets are embedded. Navigation and local data actions work offline. Receiving form submissions needs a backend; use a private publication for saved responses.', 'تُدمج الملفات داخل التصدير. تعمل الروابط والبيانات المحلية دون اتصال. استقبال ردود النماذج يحتاج خادمًا؛ استخدم النشر الخاص لحفظ الردود.') : format === 'film' ? t('The browser records the composition in real time. Keep this tab visible until it finishes.', 'يسجل المتصفح المشهد في الزمن الفعلي. أبقِ النافذة ظاهرة حتى انتهاء التصدير.') : t('Export is free. The original project and imported files stay available.', 'التصدير مجاني. يظل المشروع والملفات الأصلية محفوظة.')}</p>{format === 'film' && <Field label={t('Duration (seconds)', 'المدة بالثواني')} value={duration} onChange={setDuration} type="number" min={1} max={180}/>}<div className="row"><Button disabled={busy || !project.revision} onClick={run}><Download size={15}/>{busy ? t('Exporting…', 'جار التصدير…') : t('Export', 'تصدير')}</Button>{busy && format === 'film' && <Button variant="outline" onClick={() => abort?.abort()}>{t('Cancel', 'إلغاء')}</Button>}</div>{busy && format === 'film' && <Progress value={progress * 100}/>}
 {format==='source'&&<><Button variant="outline" disabled={busy||!project.revision} onClick={inspectSource}><Code2 size={16}/>{t('Browse source before downloading','تصفح الكود قبل التنزيل')}</Button>
 {sourceFiles.length>0&&<section className="source-code"><Choice label={t('Project files','ملفات المشروع')} value={sourcePath} onChange={setSourcePath} options={sourceFiles.map(f=>({value:f.name,label:f.name}))}/><textarea aria-label={t('File contents','محتوى الملف')} readOnly spellCheck={false} dir="ltr" value={viewedFile?.content.slice(0,30000)||''}/>{(viewedFile?.content.length||0)>30000&&<p className="muted">{t('Showing the first 30,000 characters. Copy and download include the complete file.','تُعرض أول 30 ألف محرف. النسخ والتنزيل يشملان الملف كاملًا.')}</p>}<div className="row"><Button variant="outline" disabled={!viewedFile} onClick={async()=>{try{await navigator.clipboard.writeText(viewedFile!.content);toast.success(t('File copied','نُسخ الملف'));}catch{toast.error(t('Copy unavailable. Download the file instead.','النسخ غير متاح. نزّل الملف بدلًا من ذلك.'));}}}><Copy size={16}/>{t('Copy full file','نسخ الملف كاملًا')}</Button><Button variant="outline" disabled={!viewedFile} onClick={()=>downloadBlob(new Blob([viewedFile!.content],{type:'text/plain;charset=utf-8'}),viewedFile!.name.split('/').pop()!)}><Download size={16}/>{t('Download file','تنزيل الملف')}</Button></div></section>}</>}
 </div>; }
