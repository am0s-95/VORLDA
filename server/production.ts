...
import { indexStoredObjects,storageTotalSQL } from './storage-accounting.ts';
import { entitlement, reserveStorage } from './entitlements.ts';
...
const activeStates="('reserved','starting','running','delivering','uncertain','delivery_failed')";
const OUTPUT_BUDGET=25*1024*1024;
...
export async function failTask(env:Env,t:Task,message:string){
    ...
    await db(env).batch([
        ...
        db(env).prepare('DELETE FROM resource_reservations WHERE id=? AND EXISTS(SELECT 1 FROM ledger WHERE id=?)').bind('production:'+t.id,nonce)
    ]);
}
async function deliveryIssue(env:Env,t:Task,message:string){await db(env).prepare("UPDATE production_tasks SET status='delivery_failed',error=?,updated_at=? WHERE id=? AND status IN ('running','delivering','delivery_failed')").bind(message+' Generation was already performed. Retry delivery without a new generation charge, or request owner review.',now(),t.id).run();}
async function deliver(env:Env,t:Task,prediction:any){
    ...
    let url:URL;try{url=new URL(output);}catch{await deliveryIssue(env,t,'The provider returned no usable image.');return;}
    if(url.protocol!=='https:'||!(url.hostname==='replicate.delivery'||url.hostname.endsWith('.replicate.delivery'))){await deliveryIssue(env,t,'The provider returned an unsupported image address.');return;}
    // One delivery lease. An interrupted lease can safely be reclaimed after two minutes.
    const stamp=now(),claim=await db(env).prepare("UPDATE production_tasks SET status='delivering',updated_at=? WHERE id=? AND (status IN ('running','delivery_failed') OR (status='delivering' AND updated_at<?))").bind(stamp,t.id,new Date(Date.now()-120000).toISOString()).run();if(!claim.meta.changes)return;
    ...
    try{
        ...
        const prepaidSpace=await db(env).prepare('SELECT id FROM resource_reservations WHERE id=?').bind('production:'+t.id).first<any>();
        reservation=prepaidSpace?.id||await reserveStorage(env,t.owner,t.project_id,size);const bytes=new Uint8Array(size);let offset=0;for(const chunk of chunks){bytes.set(chunk,offset);offset+=chunk.byteLength;}
        ...
        await db(env).batch([
            ...
            db(env).prepare("UPDATE production_tasks SET status='completed',output=?,error=NULL,updated_at=? WHERE id=? AND status='delivering'").bind(result,now(),t.id),
            db(env).prepare('DELETE FROM resource_reservations WHERE id=?').bind(reservation)
        ]);
    }catch(e){
        // Provider work already succeeded. Keep the debit and any storage lease; a D1
        // timeout may have committed the asset, so never delete its object here.
        await deliveryIssue(env,t,e instanceof Error?e.message:'Image delivery failed.');
    }
}
...
export async function productionApi(request:Request,env:Env,user:User):Promise<Response|null>{
    ...
    if(path==='/api/admin/production'&&method==='GET'){requireAdmin(user);return json((await db(env).prepare("SELECT id,project_id,kind,status,amount,error,created_at FROM production_tasks WHERE status IN ('uncertain','delivery_failed') OR (status='starting' AND updated_at<?) ORDER BY created_at DESC LIMIT 100").bind(new Date(Date.now()-120000).toISOString()).all()).results);}
    const review=path.match(/^\/api\/admin\/production\/([-\w]+)\/refund$/);
    if(review&&method==='POST'){requireAdmin(user);const b=await body(request,4000),t=await task(env,review[1]);if(!t)throw new ApiError(404,'Operation not found.');if(typeof b.reason!=='string'||b.reason.trim().length<10)throw new ApiError(400,'Record the reason for this reviewed refund.');if(!['uncertain','starting','failed','delivery_failed'].includes(t.status))throw new ApiError(409,'This operation is not awaiting manual settlement.');if(t.status==='starting'&&Date.now()-Date.parse(t.updated_at)<120000)throw new ApiError(409,'The provider may still be acknowledging this operation.');await failTask(env,t,'Owner-reviewed refund: '+b.reason.trim().slice(0,1000));return json({reviewed:true});}
    ...
    if(action==='execute'&&method==='POST'){
        ...
        await indexStoredObjects(env,p.owner);
        const outputBytes=input.kind==='image'?OUTPUT_BUDGET:0;
        const result=await db(env).batch([
            db(env).prepare(`INSERT INTO production_tasks(id,owner,actor,project_id,request_id,mode,kind,status,input,amount,from_subscription,from_topup,created_at,updated_at) SELECT ?,?,?,?,?,?,?,'reserved',?,?,MIN(w.subscription,?),?-MIN(w.subscription,?),?,? FROM wallets w JOIN quotes q ON q.id=? WHERE w.owner=? AND w.mode=? AND w.subscription+w.topup>=? AND q.used=0 AND q.expires_at>? AND (SELECT COUNT(*) FROM production_tasks WHERE owner=? AND mode=? AND status IN ${activeStates})<? AND (?=1 OR COALESCE((SELECT SUM(amount) FROM production_tasks WHERE actor=? AND project_id=? AND created_at>=? AND status!='failed' AND mode=?),0)+?<=?) AND (${storageTotalSQL})+?<=? ON CONFLICT(actor,request_id) DO NOTHING`).bind(taskId,p.owner,user.id,p.id,key,mode,input.kind,q.payload,q.amount,q.amount,q.amount,q.amount,stamp,stamp,key,p.owner,mode,q.amount,Date.now(),p.owner,mode,ent.concurrency,p.role==='owner'?1:0,user.id,p.id,month,mode,q.amount,allowance,p.owner,p.owner,p.owner,p.owner,outputBytes,ent.storageBytes),
            db(env).prepare(`INSERT INTO resource_reservations(id,owner,project_id,bytes,created_at) SELECT ?,?,?,?,? WHERE ?>0 AND ${condition}`).bind('production:'+taskId,p.owner,p.id,outputBytes,stamp,outputBytes,taskId),
        ...
        if(!result[0].meta.changes){const winner=await db(env).prepare('SELECT * FROM production_tasks WHERE actor=? AND request_id=?').bind(user.id,key).first<Task>();if(winner)return json(winner);throw new ApiError(409,'Insufficient paid balance, output storage, spending allowance, or available processing slots. No charge was made.');}
    ...
    if(action&&method==='GET'){
        ...
        if(t.provider_id&&['running','delivering','delivery_failed'].includes(t.status)){const result=await provider(env,'predictions/'+encodeURIComponent(t.provider_id));await deliver(env,t,result);}
