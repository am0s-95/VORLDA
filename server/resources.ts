...
export async function resourceApi(request:Request,env:Env,user:User):Promise<Response|null>{
    ...
    if(method==='GET'&&!key){const rows=await db(env).prepare('SELECT * FROM studio_resources WHERE owner=? ORDER BY updated_at DESC').bind(user.id).all<any>();return json({items:rows.results.map(x=>({...x,data:JSON.parse(x.data)})),entitlement:await entitlement(env,user.id)});}
    ...
    if(method==='POST'&&!key||method==='PUT'&&prior){
        const b=await body(request,1500000),kind=prior?.kind||b.kind;
        ...
        const data=JSON.stringify(validateData(kind,b.data)),name=cleanName(b.name,100),time=now();
        ...
        const newId=id(),r=await db(env).prepare('INSERT INTO studio_resources(id,owner,kind,name,data,revision,created_at,updated_at) SELECT ?,?,?,?,?,1,?,? WHERE (SELECT COUNT(*) FROM studio_resources WHERE owner=? AND kind=?)<?').bind(newId,user.id,kind,name,data,time,time,user.id,kind,limit).run();
