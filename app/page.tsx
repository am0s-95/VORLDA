...
export default function Workshop() {
    ...
    async function refreshWallet() { setWallet(await api('/api/wallet')); }
    ...
    async function review() { if (!project)
        ...
        await saveDraft();
        setQuote(await api(`/api/projects/${project.id}/quote`, { method: 'POST', body: { graph: graphRef.current, revision: projectRef.current!.revision } }));
    }
    catch (e) {
        toast.error((e as Error).message);
    }
 ...
 </SidebarContent><SidebarFooter><button className="sidebar-rule" onClick={() => setModal('rules')}><SlidersHorizontal size={15}/><span>{t('Context & rules', 'السياق والقواعد')}</span><span className="counter">{g.rules.length}</span></button><div className="sidebar-account"><span className="avatar">{user?.name?.slice(0, 1) || 'V'}</span><div><strong>{user?.name || t('Your workspace', 'مساحتك')}</strong><small>{t('Private workshop', 'ورشة خاصة')}</small></div>{user?.admin && <IconButton label={t('Pricing settings', 'إعدادات الأسعار')} onClick={() => setModal('settings')}><Settings2 size={15}/></IconButton>}</div></SidebarFooter></Sidebar>
 ...
 <footer className="prompt-dock"><div className="prompt-context"><Sparkles size={16}/><span>{selected.length ? current?.name : scope ? activeScope?.name : t('Whole workshop', 'الورشة كاملة')}</span><span className="provider-label">{t('Provider not connected', 'المزوّد غير مربوط')}</span></div><div className="prompt-row"><Input value={prompt} aria-label={t('Contextual instruction', 'أمر سياقي')} onChange={e => setPrompt(e.target.value)} placeholder={t('Describe what should happen here…', 'صف ما تريد تنفيذه هنا…')} onKeyDown={e => { if (e.key === 'Enter')
        attachPrompt(); }}/><IconButton label={t('Attach prompt to target', 'ربط الأمر بالهدف')} disabled={!prompt.trim() || !editable} onClick={attachPrompt}><CornerDownLeft size={17}/></IconButton><span className="divider"/><div className="review-summary"><span>{dirty ? t('Draft changes', 'تعديلات مسودة') : t('Up to date', 'لا توجد تعديلات')}</span><strong>{dirty ? money(estimated) : t('Ready', 'جاهز')}</strong></div><Button disabled={!editable || !dirty} onClick={review}>{t('Review & apply', 'مراجعة واعتماد')}<ArrowUpRight size={14}/></Button></div><div className="dock-footnote"><span>{t('Draft freely. Review the cost before you apply.', 'صمّم بحرية في المسودة. راجع التكلفة قبل الاعتماد.')}</span><span>{wallet.mode === 'test' ? t('Example rates · test balance', 'أسعار مثال · رصيد تجريبي') : t('USD wallet', 'محفظة بالدولار')}</span></div></footer>
