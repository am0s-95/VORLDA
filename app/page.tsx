...
export default function Workshop() {
 ...
 <Modal open={modal === 'code'} onClose={()=>{if(!codeDirty || window.confirm(t('Discard the unsaved file edit?', 'تجاهل تعديل الملف الذي لم تحفظه؟')))setModal('');}} title={t('Programming workspace', 'مساحة البرمجة')} description={t('Source files, runtime requirements and build task nodes.', 'ملفات المصدر ومتطلبات التشغيل ونودز مهام البناء.')} wide>{modal==='code' && <Suspense fallback={<p>{t('Opening code workspace…','جار فتح مساحة البرمجة…')}</p>}><CodeWorkbench graph={g} selected={selected} editable={editable} edit={edit} choose={choose} ar={ar} onDirtyChange={setCodeDirty}/></Suspense>}</Modal>
