...
import { WorkshopNavigation, CloseMobileTools } from '@/components/workshop-navigation';
...
export default function Workshop() {
 ...
 <Modal open={modal === 'delivery'} onClose={()=>setModal('')} title={t('Export & publish', 'التصدير والنشر')} description={t('Preview, apply, then share or download your application.', 'عاين تطبيقك واعتمد تعديلاته، ثم شاركه أو نزّل مصدره.')} wide>{project && <DeliveryPanel project={project} ar={ar} dirty={dirty} busy={busy} editable={editable} apply={review} publish={publish} preview={()=>{setPreviewDraft(true);setPreview(true);}} exportSource={()=>setModal('export')} manage={()=>setModal('project')}/>}</Modal>
 ...
 <article><Play/><h3>{t('See changes immediately', 'شاهد التعديلات فورًا')}</h3><p>{t('Preview opens the current draft. Compare it with the applied version and switch between phone and desktop sizes.', 'المعاينة تفتح المسودة الحالية. قارنها بالنسخة المعتمدة وبدّل بين مقاس الهاتف والحاسوب.')}</p><Button variant="outline" disabled={!project} onClick={()=>{setPreviewDraft(true);setPreview(true);}}>{t('Open preview', 'افتح المعاينة')}</Button></article>
